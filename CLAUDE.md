...
遇到 API 限制、浏览器兼容性、技术方案不确定时，**先查文档和社区实践再动手**，不要凭猜测反复试错。

- 不确定 API 是否支持某个用法 → 查 MDN / Chrome Developer 文档 / Stack Overflow，找到确切答案后再写
- 用户反馈某种做法在系统里正常、但代码实现不 work → 先去理解系统原生做法为什么能 work，差距在哪里，再改
- 连续两次尝试都失败 → 停下来，查找权威信息，说明发现了什么，再提出修正方案
- 不要闭门造车：要参照外部权威信息（官方文档、社区讨论、已验证的示例代码），而不是猜了又猜

检验标准：每次技术决策能引用一条外部权威来源，而不是"我觉得应该可以"。
