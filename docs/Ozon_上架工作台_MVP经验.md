...
“自动品类匹配”要仿照运营人员的判断过程：采集数据只是证据，真正的分类选择交给 AI 做，但 AI 只能在程序给出的 Ozon 候选类目里选，不能自由编 ID。

当前流程：

- 程序加载 Ozon 类目树和已有翻译。
- AI 逐层选择最像运营判断的候选类目。
- 程序用 `/v1/description-category/attribute` 验证最终 `description_category_id` / `type_id` 是否能拉到属性。
- 验证通过后，程序再拉取该类目的官方属性和字典值。
- 自动填充只基于这批官方属性生成属性值。

WALLET 系列可作为历史经验和测试样本，但不能替代 AI 品类判断：

- `description_category_id`: `17027904`
- `type_id`: `93338`
- 类目: `Галантерея и аксессуары > Аксессуары > Кошелек`

换句话说，钱包规则只能帮助我们发现异常，不能绕过“AI 选类目 -> API 拉属性 -> AI/程序填属性”的链路。
